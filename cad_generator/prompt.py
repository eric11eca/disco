...
def build_problems_insertion(args, cache):
    ...
    for data in tqdm(train_subset):
        ...
        for i, span in enumerate(spans_to_mask):
            guid = f"{data['guid']}_{i}"
            if args.mode == "premise":
                prompt = f"{premise.replace(span, '[insert]')} Based on the above context, {label_new} {hypothesis}".split(
                    '[insert]')
            else:
                prompt = f"{premise}\n Based on the above context, {label_new} {hypothesis.replace(span, '[insert]')}".split(
                    '[insert]')
