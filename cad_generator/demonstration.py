...
def build_masked_nli_perturbation(args):
    examples = []
    counter_data = read_jsonl(args.demo_pth)
    label_new = label_map[args.to_label]
    ...
    for data in counter_data:
        ...
        if args.mode == "premise":
            ...
            prompt = f"story: {premise}\n conclusion: {hypothesis.replace(span_prev, '[blank]')}\n [blank] should be:"
