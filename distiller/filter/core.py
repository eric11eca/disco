...
class SentencePairModelFilter(BaseFilter):
    def tensorize(self, original, counter, teacher):
        tokenizer = teacher.tokenizer
        input_seqs = [tokenizer(x[0],x[1]).input_ids for x in original]
        counter_seqs = [tokenizer(x[0],x[1]).input_ids for x in counter]

        # orig_s1 = np.array(original)[:,0].tolist()
        # orig_s2 = np.array(original)[:,1].tolist()
        # counter_s1 = np.array(counter)[:,0].tolist()
        # counter_s2 = np.array(counter)[:,1].tolist()
    ...
    def predict(self, original, counter, teacher):
        input_seq_pair, counter_seq_pair = self.tensorize(
            original, counter, teacher)
    ...
    def encode_label(self, label, to_label):
        ...
        # prev_label_ids.to(self.device), new_label_ids.to(self.device)
        return prev_label_ids, new_label_ids
    ...
    def run(self, batch, cache, **kwargs):
        ...
        for model_name in kwargs["model_names"]:
            if model_name not in self.models:
                teacher = self.load_model(model_name)
